...
def getLargestEarnings():
    ...
    for i in range (0,5):
        day_dict = {}
        premarket = supabase.table("Earnings").select("*").eq("date", str(i)).eq("hour", "bmo").order("cap", desc=True).limit(10).execute()
        after_hours = supabase.table("Earnings").select("*").eq("date", str(i)).eq("hour", "amc").order("cap", desc=True).limit(10).execute()
        all_sessions = []
        all_sessions.append(premarket.data)
        all_sessions.append(after_hours.data)
        ...
        #print(premarket.data[0])
    #print(compiled_earnings)
...
class EarningsData:
  def __init__(self, ticker, eps_est, rev_est, eps_act, rev_act, hour, cap, date):
    ...
    self.date = date
  def __str__(self):
      return str(self.date) + str(self.ticker) + str(self.cap) + str(self.hour) + str(self.rev_act) + str(self.eps_act) + str(self.eps_est) + str(self.rev_est)
...
def format_data(ticker_info):
    ...
    for ticker in ticker_data:
        ...
        obj = EarningsData(ticker.get("symbol"), ticker.get("epsEstimate"), ticker.get("revenueEstimate"), ticker.get("epsActual"),
